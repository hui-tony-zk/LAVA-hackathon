...
ind_predictors <- all_files_clean_marks %>% 
  ...
  filter(course == 11) %>%
  group_by(instructor, semester, measure, StudentPerformance.SectionAverage, StudentPerformance.StandardError) %>%
...
```{r width=10}
ind_predictors %>%
  filter(measure %in% ind_predictors_variability$measure[1:3]) %>%
